...
void Mort::DEMO()
{
	...
	Mat org_img = imread("C:\\Users\\mikke_000\\Dropbox\\SDU\\FF501\\repo\\Automatic-Vehicle-Ticketing-System-Using-Computer-Vision-Techniques\\code\\images\\DSC_0566.jpg", IMREAD_COLOR);         //reading in image as a 3 channel (3 bytes) BGR one
	...
	for (Rect rect : boundRects)
	{
		...
		imwrite(string("object") + to_string(i) + string(".png"), org_img(rect));
...
void Mort::OWN()
{
	...
	Mat img;
	int biAmount = 0;
	bilateralFilter(org_img, img, biAmount, biAmount * 2, biAmount / 2); //setting diameter, sigma color and sigma space to their recommended values
	...
	//STEP 5: transforming image to remove holes
	Mat elm = getStructuringElement(MORPH_RECT, Size(17, 6));         //setting kernel size to these values is by trial and error
	morphologyEx(img, img, MORPH_CLOSE, elm);                      //tends to remove black holes;
																   //this is because digits in number plate are black and we are looking for plate
	imshow("Morphology close operation, press any key to continue!", img);
	waitKey(0);
	...
	for (short i = 0; i< vect_keypoints.size();)
	{
		if (vect_keypoints[i].size() >= 12) //this cutoff value is by error and trial
		{
			vect_keypoints[i].erase(vect_keypoints[i].begin() + 12, vect_keypoints[i].end());
			i++;
		}
		else
		{
...
void Mort::myContrastBrightness(Mat& image, double alpha, int beta)
{
